...
class FeatureClassificationModel(Algorithm):
    ...
    def process_batch(self, batch, do_train=True):
        ...
        # ***************** SET TORCH VARIABLES ******************
        dataX_var = Variable(
            dataX, volatile=((not do_train) or (not finetune_feat_extractor))
        )
        labels_var = Variable(labels, requires_grad=False)
        # ********************************************************

        # ************ FORWARD PROPAGATION ***********************
        feat_var = self.networks["feat_extractor"](
            dataX_var, out_feat_keys=out_feat_keys
        )
        ...
        if isinstance(pred_var, (list, tuple)):
            ...
            for i in range(len(pred_var)):
                ...
                loss_total = (
                    loss_this if (loss_total is None) else (loss_total + loss_this)
                )
                record["prec1_c" + str(1 + i)] = accuracy(
                    pred_var[i].data, labels, topk=(1,)
                )[0][0]
                record["prec5_c" + str(1 + i)] = accuracy(
                    pred_var[i].data, labels, topk=(5,)
                )[0][0]
        else:
